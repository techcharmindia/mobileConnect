...
-- Prevent one physical device from being received twice while still allowing
-- blank IMEI/serial values for ordinary non-serialized products.
CREATE UNIQUE INDEX IF NOT EXISTS idx_pos_products_imei_unique
  ON pos_products (imei) WHERE imei IS NOT NULL AND imei <> '';

CREATE UNIQUE INDEX IF NOT EXISTS idx_pos_products_serial_unique
  ON pos_products (serial) WHERE serial IS NOT NULL AND serial <> '';

-- Quick search also covers barcode, brand, model and supplier values.
CREATE INDEX IF NOT EXISTS idx_pos_products_upc_trgm
  ON pos_products USING gin (upc gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_pos_products_brand_trgm
  ON pos_products USING gin (brand gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_pos_products_model_trgm
  ON pos_products USING gin (model gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_pos_products_supplier_trgm
  ON pos_products USING gin (supplier gin_trgm_ops);
