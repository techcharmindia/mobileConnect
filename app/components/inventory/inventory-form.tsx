...
const inventoryColumns: (keyof InventoryRecord)[] = [
  "product",
  "identifier",
  "stockCount",
  "lowStockAlert",
...
const inventoryLabels: Record<keyof InventoryRecord, string> = {
  product: "Product",
  identifier: "Serial / IMEI / Code",
  stockCount: "Live stock",
  lowStockAlert: "Low stock alert",
  stockHistory: "Stock history",
};
const productIdentifiers: Record<
  InventoryProduct,
...
export default function InventoryForm() {
  ...
  function submitInventory(event: FormEvent<HTMLFormElement>) {
    ...
    setInventory((entries) => [
      ...entries,
      {
        ...
        identifier: String(values.get("productIdentifier")),
        stockCount: String(values.get("stockCount")),
        lowStockAlert: String(values.get("lowStockAlert")),
        stockHistory: String(values.get("stockHistory")),
      },
    ]);
  ...
  function downloadInventoryCsv() {
    const csv = [
      [...inventoryColumns.map((column) => inventoryLabels[column])],
      ...inventory.map((entry) =>
        inventoryColumns.map((column) => entry[column]),
      ),
    ]
      .map((row) =>
    ...
    link.download = "inventory-entries.csv";
    link.click();
          ...
          <div className="form-grid col-12">
            <label className="field col-4">
              ...
              <select
                ...
                value={product}
                onChange={(event) =>
                  setProduct(event.target.value as InventoryProduct)
                }
              >
        ...
        <div className="form-section">
          <div className="form-grid col-12">
            ...
            <label className="field col-4">
              ...
              <select name="stockHistory" required defaultValue="">
              ...
              <thead>
                <tr>
                  {inventoryColumns.map((column) => (
                    <th key={column}>{inventoryLabels[column]}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {inventory.map((entry, index) => (
                  <tr key={`${entry.identifier}-${index}`}>
                    {inventoryColumns.map((column) => (
