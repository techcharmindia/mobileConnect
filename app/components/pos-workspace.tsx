...
export default function PosWorkspace() {
  ...
  const [section, setSection] = useState(
    () => searchParams.get("section") || "Products",
  );
  ...
  useEffect(() => {
    const saved = localStorage.getItem("pos-product-filter");
    if (!saved) return;

    let timer: number | undefined;
    try {
      const filter = JSON.parse(saved) as Record<string, unknown>;
      timer = window.setTimeout(() => {
        setFilterId(typeof filter.filterId === "string" ? filter.filterId : "");
        setSearch(typeof filter.search === "string" ? filter.search : "");
        setCategory(typeof filter.category === "string" ? filter.category : "");
        setBrand(typeof filter.brand === "string" ? filter.brand : "");
        setModel(typeof filter.model === "string" ? filter.model : "");
        setSku(typeof filter.sku === "string" ? filter.sku : "");
        setImei(typeof filter.imei === "string" ? filter.imei : "");
        setSerial(typeof filter.serial === "string" ? filter.serial : "");
        setSupplier(typeof filter.supplier === "string" ? filter.supplier : "");
        setValuationMethod(
          typeof filter.valuationMethod === "string" ? filter.valuationMethod : "",
        );
        setCriteria(typeof filter.criteria === "string" ? filter.criteria : "");
        setHideOutOfStock(filter.hideOutOfStock === true);
      }, 0);
    } catch {
      localStorage.removeItem("pos-product-filter");
    }
    return () => window.clearTimeout(timer);
  }, []);
  ...
  function saveFilter() {
    const savedFilter = {
      filterId, search, category, brand, model, sku, imei, serial, supplier,
      valuationMethod, criteria, hideOutOfStock,
    };
    localStorage.setItem("pos-product-filter", JSON.stringify(savedFilter));
    setNotice("Current product filter saved in this browser.");
  }

  function unpinFilter() {
    localStorage.removeItem("pos-product-filter");
    setNotice("Saved product filter removed.");
  }
                  ...
                  <table className="pos-table pos-table-full">
                    <thead>
                      <tr>
                        ...
                        >
                          On-Hand {renderSortIcon("quantity")}
                        </th>
                        <th
                          className="pos-sortable"
                          onClick={() => toggleSort("stock_warning")}
                        >
                          Stock Warning {renderSortIcon("stock_warning")}
                        </th>
                        <th
                          className="pos-sortable"
