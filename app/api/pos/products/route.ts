  ...
  try {
    ...
    if (search) {
      conditions.push(`(
        ...
        OR COALESCE(category,'') ILIKE '%' || $${i} || '%'
        OR COALESCE(brand,'') ILIKE '%' || $${i} || '%'
        OR COALESCE(model,'') ILIKE '%' || $${i} || '%'
        OR COALESCE(imei,'') ILIKE '%' || $${i} || '%'
        OR COALESCE(serial,'') ILIKE '%' || $${i} || '%'
        OR COALESCE(supplier,'') ILIKE '%' || $${i} || '%'
        OR COALESCE(short_description,'') ILIKE '%' || $${i} || '%'
      )`);
      values.push(search);
      i++;
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (
      !["non-serialized", "serialized"].includes(
        body.itemType?.trim() || "non-serialized",
      ) ||
      !["New", "Used", "Refurbished"].includes(body.condition || "New")
    )
      return NextResponse.json(
        { message: "Enter a valid item type and condition." },
        { status: 400 },
      );

    if (body.itemType === "serialized" && !String(body.imei || "").trim())
      return NextResponse.json(
        { message: "An IMEI is required for serialized products." },
        { status: 400 },
      );
